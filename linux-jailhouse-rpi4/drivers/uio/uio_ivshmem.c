...
static int ivshm_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (!devm_request_mem_region(&pdev->dev, mem->addr, mem->size,
				     device_name))
		return -EBUSY;
	dev_info(&pdev->dev, "%s at %pa, size(s) %pa\n", mem->name, &mem->addr,
		 &mem->size);
	...
	if (output_section_sz > 0) {
        ...
        // pci_write_config_byte(pdev, vendor_cap + IVSHM_CFG_PRIV_CNTL, IVSHM_PRIV_CNTL_ONESHOT_INT);
	printk("IVSHM_PRIV_CNTL_ONESHOT_INT=%ld\n", IVSHM_PRIV_CNTL_ONESHOT_INT);
	/*
